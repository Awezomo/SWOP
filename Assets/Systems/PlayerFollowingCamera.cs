...
public class PlayerFollowingCamera : MonoBehaviour
{
	...
	public float xDeviationToTrigger;
	...
	private void LateUpdate()
	{
		...
		float refDeviation = t.Value.x - transform.position.x;

		Debug.Log(refDeviation);

		if (Mathf.Abs(refDeviation) > xDeviationToTrigger)
			xSpeed = refDeviation;

		transform.position += new Vector3(xSpeed, 0, 0) * Time.deltaTime; //new Vector3(t.Value.x, transform.position.y, transform.position.z);
		ParallaxComponent.PlayerPositionX = transform.position.x;
	}
